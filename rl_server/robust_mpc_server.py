    ...
    class Request_Handler(BaseHTTPRequestHandler):
        ...
        def do_POST(self):
            ...
            if ('pastThroughput' in post_data):
                ...
                if nearest_ho_type != 0:
                    # there is a nearby HO here
                    print("Nearby HO, ", time.time() - start_time)
                if nearest_ho_type == 1:
                    # increase throughput
                    future_bandwidth *= HO_FACTOR
                elif nearest_ho_type == -1 or nearest_ho_type == -2:
                    future_bandwidth /= HO_FACTOR

                with open(THRPT_FILE+'_robustMPCGT_'+sys.argv[1], 'a') as thrpt_file:
                    thrpt_file.write(str(time.time()) + ' ' + str(future_bandwidth) + '\n')
                    thrpt_file.flush()
                    thrpt_file.close()
...
def main():
    if len(sys.argv) == 2 or len(sys.argv) == 3:
        ...
        run(log_file_path=LOG_FILE + '_robustMPCGT_' + trace_file)
    else:
        run()
